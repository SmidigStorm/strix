...
class JwtServiceTest {
    ...
    void skalExtraheereBrukerIdFraClaims() {
        String brukerId = "BRUKER-001";
        String token = jwtService.generateToken(brukerId, "test@example.com", "Test", Arrays.asList("SOKER"), null);
        Claims claims = jwtService.validateToken(token);
    ...
    void skalExtrahereEmailFraClaims() {
        String email = "test@example.com";
        String token = jwtService.generateToken("BRUKER-001", email, "Test", Arrays.asList("SOKER"), null);
        Claims claims = jwtService.validateToken(token);
    ...
    void skalExtrahereNavnFraClaims() {
        String navn = "Test Bruker";
        String token = jwtService.generateToken("BRUKER-001", "test@example.com", navn, Arrays.asList("SOKER"), null);
        Claims claims = jwtService.validateToken(token);
    ...
    void skalExtrahereOrganisasjonIdFraClaims() {
        String organisasjonId = "ORG-001";
        String token = jwtService.generateToken("BRUKER-001", "test@example.com", "Test", Arrays.asList("SOKER"), organisasjonId);
        Claims claims = jwtService.validateToken(token);
    ...
    void skalReturnereNullNaarOrganisasjonIdIkkeFinnes() {
        String token = jwtService.generateToken("BRUKER-001", "test@example.com", "Test", Arrays.asList("SOKER"), null);
        Claims claims = jwtService.validateToken(token);
